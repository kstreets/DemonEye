...
namespace PrimeTween {
    ...
    internal class ReusableTween {
        ...
        internal void OnComplete([CanBeNull] Action _onComplete, bool warnIfTargetDestroyed, [CanBeNull] Func<bool> _onValidate) {
            if (_onComplete == null) {
                return;
            }
            validateOnCompleteAssignment();
            ...
            onComplete = tween => {
                var callback = tween.onCompleteCallback as Action;
                var validateCallback = tween.onValidateCallback as Func<bool>;
                Assert.IsNotNull(callback);
                try {
        ...
        internal void OnComplete<T>([CanBeNull] T _target, [CanBeNull] Action<T> _onComplete, bool warnIfTargetDestroyed, [CanBeNull] Func<T, bool> _onValidate) where T : class {
            if (_target == null || isDestroyedUnityObject(_target)) {
                Debug.LogError($"{nameof(_target)} is null or has been destroyed. {Constants.onCompleteCallbackIgnored}");
                return;
            }
            ...
            onComplete = tween => {
                var callback = tween.onCompleteCallback as Action<T>;
                var validateCallback = tween.onValidateCallback as Func<T, bool>;
                Assert.IsNotNull(callback);
                var _onCompleteTarget = tween.onCompleteTarget as T;
                if (isDestroyedUnityObject(_onCompleteTarget)) {
                    tween.warnOnCompleteIgnored(true);
